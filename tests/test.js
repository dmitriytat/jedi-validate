describe('JediValidate', function () {
    ...
    describe('Parse input name', function () {
        ...
        it('Simple Name', function () {
            assert.deepEqual(JediValidate.getInputData({ value: value, name: simpleName }), { name: value });
        });

        it('Array Name', function () {
            assert.deepEqual(JediValidate.getInputData({ value: value, name: arrayName }), { name: [value] });
        });

        it('Object Name', function () {
            assert.deepEqual(JediValidate.getInputData({ value: value, name: objectName }), { name: { a: value } });
        });

        it('Object Array Name', function () {
            assert.deepEqual(JediValidate.getInputData({ value: value, name: objectArrayName }), { name: { a: [value] } });
        });

        it('Array Object Name', function () {
            assert.deepEqual(JediValidate.getInputData({ value: value, name: arrayObjectName }), { name: [{ a: value }] });
        });
    });
});
